...
class SCARE():
	...
	incr = 1
	created = False
	choice = int(input("INTER SCRAPING OPTHION \n WITH REQUEST 1 \n WITH SELINUM 2 \n: "))
	if choice == 2 :
	...
	def scrape_p(self):
		...
		response = self.soups(self.initial_url)
	...
	def con(self,element,patern):
		...
		if match:		
			...
			elname = self.downimg(elname,"fmg1")	
		elif elname.name == 'a':
	...
	def get_P(self,cls,*ar):
		...
		for url in self.urllist:		
			...
			elements = self.soup.find_all(attrs={'class':cls})		
	...
	def downproimg(self):
		link = ""
		# self.jsonname = 'fty.json'
		tryfile = self.jsonname
		pattern = 'div.l4pr-container ul img'
		# pattern = 'div.swiper-custom-pagination img'
		...
		with open(tryfile , 'r') as file :
			...
			for i in prolink:
				...
				link = i['link']
				print(link,"NUM",incr)		
				# self.initial_url = link
				
				# response = requests.get(link.strip())
				response = self.soups(link.strip())
				...
				newimg = self.soup.select(pattern)	
				print(newimg)		
				inc = 1
				dic['link'] = link.strip()
				dic['id'] = incr
				...
				diclist.append(dic)
		with open('product_img.json','w') as new:
			json.dump(diclist,new, indent=2,ensure_ascii=False)
