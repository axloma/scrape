	...
	def scrape_p(self):
		...
		for link in links:   
			...
			try:
				...
				print(x,"XX")
				if linksid:
					if  p := link['data-next'] is not None :
						# p = link['data-next']
						print(type(p))
						print(p,"P")
					else:
...
# elclan = SCARE('https://el-clan.com/collections/disposable','load-more-button','elclan.txt')
# elclan.scrape_p()
# elclan.save()
...
elclan = SCARE('https://el-clan.com/collections/disposable','load-more-button','elclanl.txt','elclan','elclanl.json')
elclan.scrape_p()
elclan.save()
elclan.openfile()
elclan.get_P('w33','h3 span','h3 a','div p.price','h3 a','img')
